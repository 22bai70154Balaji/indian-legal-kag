...
class Article21PrivacyAnalyzer:
    ...
    def _generate_recommendations(self,
        ...
        if confidence < 60:
            recommendations.append("⚠️ Low confidence score - Manual review recommended for detailed assessment")
        
        if compliance_score < 70:
            recommendations.append("🔴 Privacy compliance below 70% - Enhanced privacy safeguards recommended")
        
        if len(privacy_clauses) < 3:
            recommendations.append("📋 Limited privacy clauses found - Consider comprehensive privacy framework review")
        
        if categorization.get("informational_privacy", {}).get("score", 0) < 0.5:
            recommendations.append("🔐 Strengthen informational privacy protections")
        
        if categorization.get("bodily_privacy", {}).get("score", 0) < 0.5:
            recommendations.append("💪 Enhance bodily integrity protections")
        
        if not recommendations:
            recommendations.append("✅ Privacy framework satisfactory - Maintain current safeguards")
